...
2. **检查工件完成状态**
   ...
   **如果有未完成的工件：**
   - 显示警告，列出未完成的工件
   - 使用 **AskUserQuestion 工具** 让用户确认是否继续
   - 用户确认后继续
...
4. **评估规范同步状态**
   ...
   **提示选项：**
   ...
   如果用户选择同步，使用 Task 工具（subagent_type: "general-purpose"，prompt: 使用 Skill 工具调用 openspec-sync-specs，并传入 delta spec 分析摘要）。无论是否同步，都继续进行归档。
